...
## 定位
自用鸿蒙 HarmonyOS NEXT **桌面 agent**（手机版 zcode）：打开即对话，agent loop 多步工具调用；@技能名 强制技能 / 自动路由可关；回答=聊天气泡或整页 HTML 画布（eli5 为内置 skill）。独立 git 仓库，纯本地（无 remote）。
...
- 单测（模拟器实跑）：`hdc shell aa test -b com.zhuolin.eli5harmony -m entry_test -s unittest OpenHarmonyTestRunner -s class logicTest`（当前 43/43）
