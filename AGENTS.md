...
- `store/` Db（conversations/messages/skills 三表）/ ConversationRepo / MessageRepo / SkillRepo / ConfigStore（Preferences，含 longModel/autoRoute/**themeMode**）
- `common/` WallpaperLayer（壁纸层）/ **ThemeMode**（主题三档：手动档 `setColorMode` 换 `$r` 资源档 + 生效明暗写 AppStorage；两套枚举不同名，必须显式转换）
