...
## 当前状态（2026-09-16）
- **09-16 M2b 收尾深化（`294e393`→`d719570`，验收报告 `tests/fullflow/reports/20260916_m2b_finalize/`）**：①屏蔽应用管理 UI——设置页屏幕感知卡片新入口行 + `ui/settings/ScreenPrivacyDialog.kt`（黑名单增删/添加列表 launcher 过滤带图标/学习集只读清空二段确认；ConfigStore 补 `clearVisionLearned()`；学习集为空时该区隐藏）；②闪问历史轻打磨——条目点开全文视图（与列表互斥渲染防 AlertDialog 叠显）+单条删除（FlashDao 补 `deleteById`，删库+刷新不关弹窗）；③manifest 补 ⑳ 写操作安全靶（propose_plan 计划卡→点执行→断言子页 `Airplane mode`——主页同名条目 Network & internet 不可作切页判据）。门1 196 绿 + 模拟器三链验收全 PASS。**计划文档** `../design-explorations/2026-09-16-m2b-finalize-m3-research-plan.md`（M3 调研=下一步，真机轮用户拍板暂缓）
- **09-16 坑补记**：**force-stop 后无障碍服务绑定留在死进程**（重装/重启 app 后 instance==null，read_screen/capture_screen 返回 40 字符 error，模型拿不到屏幕信息会放弃写链直答——表象是「模型不调 propose_plan」，根因在服务绑定）；修法=翻转 `settings put secure enabled_accessibility_services`（先置空再置回）触发 rebind。发送钮 dump bounds 是左上角，tap 须 +12px 取中心（y=548 点不中、523 才对）。空态欢迎语与消息流（计划卡）叠加透出为已知现象（09-15 验收同款），不影响点击。
