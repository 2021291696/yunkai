...
- **联网搜索双轨**：必应爬取（免 key，本地直连 cn.bing.com）/ 博查 / Tavily；智谱系模型可选用内置 `web_search` 工具（注意：实测 glm-4.7 不执行该工具，glm-4.5 可用）
- **多轮追问**：历史只回传「问题+纯文本要点」控成本；模型需要某轮全文时输出 `[[READ_HTML:轮次号]]` 由 app 注入重发（最多一次）
- **历史会话**：本地 RDB 存储，重进恢复渲染，长按删除
- **eli5 兜底**：非合法 HTML 自动重试一次
...
模拟器安装免签名：`hdc install entry/build/default/outputs/default/entry-default-unsigned.hap`（真机调试需在 File > Project Structure > 签名配置 开自动签名）。
